...
Writes:
    Filter/results/aggregated/<method>_aggregated.csv
    Filter/results/aggregated/aggregated_summary.txt
"""
...
def aggregate_method(method):
    ...
    if method == "variance":
        group_cols = ["Cancer", "Method", "Threshold"]
    else:
...
def _format_block(method, summary):
    ...
    for _, row in summary.iterrows():
        ...
        if "Threshold" in row:
            header += f"  Threshold: {row['Threshold']}"
        lines.append(header)
...
def main():
    methods = sys.argv[1:] if len(sys.argv) > 1 else ["anova", "mutual_info", "variance"]
    ...
    with open(txt_path, "w") as f:
        f.write("# Aggregated Filter Method Results\n")
        for m in methods:
