...
plt.title(f"{CANCER} - Mutual Information Filter Confusion Matrix")
plt.ylabel("True Label")
plt.xlabel("Predicted Label")
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/mutual_info_confusion_matrix.png", dpi=150)
plt.close()
...
plt.ylim(0, 1.1)
plt.title(f"{CANCER} - Mutual Information Filter Performance")
plt.ylabel("Score")
plt.grid(axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/mutual_info_metrics_bar.png", dpi=150)
plt.close()
...
results.to_csv(f"{OUT_DIR}/mutual_info_filter_results.csv", index=False)

with open(f"{OUT_DIR}/mutual_info_results.txt", "w") as f:
    f.write(f"=== {CANCER} Mutual Information Filter ===\n\n")
