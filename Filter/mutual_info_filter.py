...
# Main Runner
# -----------------------------
def run_mutual_info_filter(cancer_type="BRCA", seed=RANDOM_STATE, save_plots=True, use_cache=True, n_jobs=-1):
    ...
    plt.savefig(
        os.path.join(
            OUT_DIR,
            f"{cancer_type.lower()}_mutual_info_confusion_matrix.png"
        ),
        dpi=150
    )
    ...
    plt.savefig(
        os.path.join(
            OUT_DIR,
            f"{cancer_type.lower()}_mutual_info_metrics_bar.png"
        ),
        dpi=150
    )
    ...
    results.to_csv(
        os.path.join(
            OUT_DIR,
            f"{cancer_type.lower()}_mutual_info_filter_results.csv"
        ),
        index=False
    )
