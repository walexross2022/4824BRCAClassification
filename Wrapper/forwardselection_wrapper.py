...
# -----------------------------
# STEP 2: Run Forward Selection Sweep
# -----------------------------
all_results = []

# Limit features via a pre-filter
variances = X_train.var()
top_features = variances.nlargest(2000).index
...
for fraction in FEATURE_FRACTIONS:
    ...
    print("\n" + "=" * 50)
    print(f"Running Forward Selection: fraction={fraction}  "
          f"n_features={n_features_to_select}")
    print("=" * 50)

    start_time = time.time()

    # Scale first — forward selection uses a model internally
...
results.to_csv(f"{OUT_DIR}/forward_selection_results.csv", index=False)
...
plt.title(f"{CANCER} - Forward Selection Sweep")
...
plt.legend()
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/forward_selection_sweep.png", dpi=150)
plt.close()
...
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/forward_selected_features.png", dpi=150)
plt.close()

# -----------------------------
# STEP 6: Save Summary TXT
# -----------------------------
with open(f"{OUT_DIR}/forward_selection_results.txt", "w") as f:
    f.write(f"=== {CANCER} Forward Selection Sweep ===\n\n")
...
print(f"\n[{CANCER}] Done - forward selection sweep saved to {OUT_DIR}/")