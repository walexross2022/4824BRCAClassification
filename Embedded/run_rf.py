...
def run_rf_workflow(cancer_type, seed, save_plots=True, use_cache=True, n_jobs=-1):
    print(f"\n=== Starting Random Forest Workflow for {cancer_type} (Seed {seed}) ===")
    ...
    grid_search = GridSearchCV(
        rf, param_grid, cv=StratifiedKFold(3, shuffle=True, random_state=seed),
...
if __name__ == "__main__":
    ...
    for cancer in cancers:
        ...
        for run_idx, seed in enumerate(seeds):
            res = run_rf_workflow(cancer, seed)
            res["run"] = run_idx + 1
            all_runs.append(res)
    ...
    print("\n[DONE] All Random Forest runs completed (10 runs per cancer).")
